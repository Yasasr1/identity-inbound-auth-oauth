...
/**
 * This class is used when response_mode = form_post.jwt=
 */
public class FormPostJwtResponseModeProvider extends JarmResponseModeProvider {

    private static final String RESPONSE_MODE = OAuthConstants.ResponseModes.FORM_POST_JWT;
    private static final Log LOG = LogFactory.getLog(FormPostJwtResponseModeProvider.class);
    ...
    public String getAuthResponseBuilderEntity(AuthorizationResponseDTO authorizationResponseDTO) {
        ...
        try {
            ...
            String params = buildParams(jwtToken);

            htmlForm = createBaseFormPage(params, authorizationResponseDTO.getRedirectUrl(),
                    authorizationResponseDTO.getFormPostRedirectPage());
        } catch (Exception e) {
            LOG.error("Error occurred when getting JWT token ", e);
            String params = buildErrorParams(authorizationResponseDTO);
            htmlForm = createBaseFormPage(params, authorizationResponseDTO.getRedirectUrl(),
                    authorizationResponseDTO.getFormPostRedirectPage());
        }
    ...
    private String buildParams(String jwtToken) {

        StringBuilder paramStringBuilder = new StringBuilder();
        paramStringBuilder.append("<input type=\"hidden\" name=\"response\" value=\"")
                .append(jwtToken)
                .append("\"/>\n");
