...
public class AgentCibaUserValidator implements CibaUserValidator {
    ...
    @Override
    public boolean appliesTo(CibaAuthCodeRequest cibaAuthCodeRequest) {
    ...
    public void validate(CibaUserResolver.ResolvedUser resolvedUser, CibaAuthCodeRequest cibaAuthCodeRequest,
        ...
        if (StringUtils.isBlank(ownerClaimName)) {
            throw new CibaClientException("The agent owner claim (" + CibaConstants.AGENT_OWNER_CLAIM_URI +
                    ") is not mapped in the OIDC dialect. Cannot validate agent owner binding.");
        }

        String agentOwner = actorTokenClaims.get(ownerClaimName);
        if (StringUtils.isBlank(agentOwner)) {
            throw new CibaClientException("The agent owner claim is missing in the actor token. " +
                    "Cannot validate agent owner binding.");
        }
